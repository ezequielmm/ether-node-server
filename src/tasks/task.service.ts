...
import { createMoreOrEquals12NodesMap, createLess12NodesMap } from 'src/game/map/builder/actOne.config';
...
export class TaskService {
    ...
    async handleMapCreation(): Promise<void> {
        ...
        const maxNodeSteps = this.configService.get<number>('AUTOGENERATED_MAP_MAX_STEPS');
        let map;
        ...
        if(maxNodeSteps >= 12){
            map = await this.mapBuilderService.createMap({
                actConfig: createMoreOrEquals12NodesMap(maxNodeSteps),
                makeAvailable: true
            });
        }else{
            map = await this.mapBuilderService.createMap({
                actConfig: createLess12NodesMap(maxNodeSteps),
                makeAvailable: true
            });
        }
