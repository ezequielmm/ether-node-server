...
export const HolyExplosionCardUpgraded: Card = {
    ...
    description: `Deal X + ${5} + {${resolveStatus.name}} and inflict X + 3 ${burn.name} to all Undead enemies. Deal X + 3 ${damageEffect.name} + {${resolveStatus.name}} and inflict 3 ${burn.name} to all other enemies. `,
    keywords: [],
    properties: {
        effects: [
            {
...
export const HolyExplosionCard: Card = {
    ...
    pool: 'knight',
    energy: CardEnergyEnum.All,
    description: `Deal X + ${3} + {${resolveStatus.name}} and inflict 3 ${burn.name} to all Undead enemies. Deal X + 2 ${damageEffect.name} + {${resolveStatus.name}} and inflict 2 ${burn.name} to all other enemies`,
    keywords: [],
    properties: {
        effects: [
            {
